...
- **Module path resolution** — `ResolveModulePath` resolves `./` and `../` paths relative to the importing file's directory, then expands to an absolute path. Absolute paths are used as cache keys to prevent loading the same file via different relative paths. Bare specifiers (without `./` or `../`) are rejected with a clear error.
