...
- **TOML compatibility target** — The project goal for TOML is full TOML 1.1.0 compatibility. A rerun against the official `toml-test` TOML 1.1.0 file list on 2026-04-04 matched the expected parse/fail result and tagged-JSON output for all 680 of 680 cases, with 0 false accepts, 0 false rejects, and 0 valid-case mismatches. The official rerun command is `python3 scripts/run_toml_test_suite.py`.
