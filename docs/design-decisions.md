...
- **JSON5 compatibility target** — The project goal for JSON5 is full parser compatibility with the reference `json5/json5` implementation plus upstream-aligned stringify behavior. The local parser rerun command is `python3 scripts/run_json5_test_suite.py`, or `python3 scripts/run_json5_test_suite.py --harness=./build/GocciaJSON5Check` if you already built the decoder harness. A rerun against the upstream parser test corpus on 2026-04-05 matched 84 of 84 extracted parser cases, and runtime stringify regressions are covered in `tests/built-ins/JSON5/stringify.js` for special numeric values, quote handling, replacers, options objects, and pretty-print trailing commas.
