...
- **JSONL module imports** — `.jsonl` modules intentionally expose each non-empty line as a zero-based string-indexed named export (`"0"`, `"1"`, ...). This keeps the structured-data import surface consistent with the existing string-literal named import/export work and means namespace imports can reuse the same export table without introducing a JSONL-specific wrapper object just for modules.
- **TOML parser split** — `Goccia.TOML` provides a standalone `TGocciaTOMLParser` utility that converts TOML text into `TGocciaValue` trees. The module loader reuses that parser for `.toml` imports, exposing each root-table key as a named export, and namespace imports simply project that same export table into a frozen namespace object. TOML date/time values currently surface as validated strings rather than Temporal values.
