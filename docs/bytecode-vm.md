...
- retain a static named import's module namespace in its local/upvalue slot: `OP_IMPORT` and resolution-only `OP_VALIDATE_IMPORT_BINDING` scale with declarations, while repeated identifier reads use `OP_GET_IMPORT_BINDING` to dereference the cached live binding identity without repeating module-loader lookup
