...
| Differential batteries | `test-cli-differential.ts` runs each `scripts/differential/` battery under interpreter, bytecode, and `bun test`, requiring both modes to agree on failed-test **name** sets and counts, and bun to agree with both; `*.goccia.test.js` batteries are goccia-only, so bun never runs them and only mode parity applies; a per-file timeout counts as a divergence. See [Differential Testing](differential-testing.md) |
