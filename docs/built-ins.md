...
| `gc` | `function` | Trigger synchronous manual garbage collection. Returns `undefined`; can run from top-level code, active function/callback frames, and CLI worker threads. Also exposes read-only `gc.bytesAllocated` (approximate GC heap size in bytes) and `gc.maxBytes` (active ceiling; defaults to half of physical memory capped at 8 GB on 64-bit or 700 MB on 32-bit, overridable via `--max-memory`). Allocations exceeding the ceiling throw a `RangeError`. |
