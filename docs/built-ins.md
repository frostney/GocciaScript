...
| `gc` | `function` | Trigger manual garbage collection from safe top-level script boundaries. Returns `undefined`; calls from worker threads or active JS function/callback frames are no-ops. Also exposes read-only `gc.bytesAllocated` (approximate GC heap size in bytes) and `gc.maxBytes` (active ceiling; defaults to half of physical memory capped at 8 GB on 64-bit or 700 MB on 32-bit, overridable via `--max-memory`). Allocations exceeding the ceiling throw a `RangeError`. |
