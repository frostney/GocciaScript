...
- **Compatibility call bindings** — With `--compat-non-strict-mode` enabled, ordinary functions, shorthand methods, accessors, and generators create an unmapped `arguments` object in their call scope unless a parameter list already binds `arguments`; arrow call scopes deliberately skip it so `arguments` remains lexical. The object is an ordinary `TGocciaObjectValue` built by `Goccia.Values.ArgumentsObjectValue`, matching the shared value-system model. Regular-function calls also coerce nullish `this` to `globalThis`; arrows keep lexical `this`.
- **`with` object environments** — With `--compat-non-strict-mode` enabled, `TGocciaWithScope` wraps the object produced by `ToObject`, checks `HasProperty`, honors `Symbol.unscopables`, and forwards reads/writes to that object before falling back to the parent scope. The scope is GC-managed rather than stack-owned because closures created inside the `with` body can retain it after the body exits.
- **Non-strict `delete`** — With `--compat-non-strict-mode` enabled, `delete identifier` uses scope `DeleteBinding` semantics (`false` for declared bindings, `true` for unresolvable names), and property deletion returns `false` for non-configurable properties instead of raising the strict-mode `TypeError`.
