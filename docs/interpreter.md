...
- **Compatibility call bindings** — With `--compat-non-strict-mode` enabled for script source, ordinary functions, shorthand methods, accessors, and generators create an unmapped `arguments` object in their call scope unless a parameter list already binds `arguments`; arrow call scopes deliberately skip it so `arguments` remains lexical. The object is an ordinary `TGocciaObjectValue` built by `Goccia.Values.ArgumentsObjectValue`, matching the shared value-system model. Ordinary function calls also coerce nullish `this` to `globalThis`; arrows keep lexical `this`. Module source creates module scopes with `NonStrictMode = False`, so the compatibility flag does not leak sloppy `arguments` or `this` semantics into modules.
...
- [Architecture](architecture.md) — Shared source frontend, both execution modes, main layers, design direction
