...
- **Compatibility call bindings** — With `--compat-non-strict-mode` enabled, ordinary functions, shorthand methods, accessors, and generators create an unmapped `arguments` object in their call scope unless a parameter list already binds `arguments`; arrow call scopes deliberately skip it so `arguments` remains lexical. The helper object is built through `Goccia.Arguments.ObjectValue` and is shared by interpreter and bytecode semantics.
