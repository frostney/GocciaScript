...
- **VMT-based chain-walking** — `FindThisValue`, `FindOwningClass`, `FindSuperClass`, and `FindNewTarget` walk the parent chain calling the corresponding virtual `Get*` method on each scope, stopping at the first non-`nil` result or at a `TGocciaCallScope` function boundary (ordinary functions do not inherit `super`, `new.target`, or owning class). `TGocciaArrowCallScope` is transparent to these walks since arrow functions inherit these bindings lexically.
