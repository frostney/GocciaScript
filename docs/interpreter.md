...
- **Compatibility call bindings** — Ordinary functions, shorthand methods, accessors, and generators create an unmapped `arguments` object in their call scope unless a parameter list already binds `arguments`; arrow call scopes deliberately skip it so `arguments` remains lexical. The helper object is built through `Goccia.Arguments.ObjectValue` and is shared by interpreter and bytecode semantics.
- **`with` object environments** — `TGocciaWithScope` wraps the object produced by `ToObject`, checks `HasProperty`, honors `Symbol.unscopables`, and forwards reads/writes to that object before falling back to the parent scope. The scope is GC-managed rather than stack-owned because closures created inside the `with` body can retain it after the body exits.
