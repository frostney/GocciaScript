...
- **Not aiming for 100% ECMAScript conformance** — Features excluded by design (`eval`, `while` / `do...while`) will not be added. `var`, the `function` keyword, `arguments`/`with`/`delete`/`this` non-strict semantics, loose equality, and the traditional `for(init; test; update)` loop are excluded by default but available as opt-in compatibility toggles (`--compat-var`, `--compat-function`, `--compat-non-strict-mode`, `--compat-loose-equality`, `--compat-traditional-for-loop`). Runtime enforcement of type annotations is also opt-in (`--strict-types`). See [Language](language.md) for the full list.
