...
// CSV imports — each data row becomes a string-indexed named export (headers on by default)
import { "0" as firstRow, "1" as secondRow } from "./data.csv";

// TSV imports — same as CSV but tab-delimited with backslash escaping
...
CSV (`.csv`) and TSV (`.tsv`) imports are supported. CSV files are parsed with RFC 4180 semantics (comma-delimited, double-quote escaping), while TSV files use IANA `text/tab-separated-values` semantics (tab-delimited, backslash escaping). Both default to headers mode — the first row becomes property names and each subsequent data row becomes an object. Each data row is exposed as a named export under its zero-based string index (`"0"`, `"1"`, ...). In runtime code, `CSV.parse(...)` / `TSV.parse(...)` return the full array, with optional reviver callbacks that receive `(key, value, { quoted, row, column })` context, and `CSV.parseChunk(...)` / `TSV.parseChunk(...)` expose streaming state.
