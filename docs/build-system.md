...
Releases are cut with the `/create-release` flow: the changelog and version bump land on a `release/<version>` branch through a squash-merged release PR *before* the tag is created, so the tag always contains its own changelog (the release commit is `chore(release): <version>`, which `cliff.toml` skips). The release job in `ci.yml` is tag-triggered and only *publishes*: it uses the [`orhun/git-cliff-action`](https://github.com/orhun/git-cliff-action) to render the latest release section (`--latest`) and creates the GitHub release with the platform archives via `softprops/action-gh-release`. It does **not** regenerate or commit the changelog — that already happened before the tag, which keeps each release self-contained. GitHub's auto-generated release notes (`.github/release.yml`) provide an additional PR-label-based view.
