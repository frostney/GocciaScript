...
**`web-tooling`** (needs build, ubuntu-latest x64 only) — Downloads the `gocciascript-x86_64-linux` build, installs the latest Node Current release as the upstream build runtime, checks out `v8/web-tooling-benchmark` at the SHA pinned in `perf/web-tooling/manifest.json`, installs upstream dependencies without running postinstall, and runs `node scripts/web-tooling-ci-report.js --web-tooling-dir web-tooling-suite --output web-tooling-report.json`. The report set is every pinned Web Tooling workload under Goccia bytecode only. Build failures and Goccia execution failures are recorded per workload in the normalized JSON instead of requiring reference-engine success. The job uploads `web-tooling-report` as a 30-day artifact. On main, `cd website && bun run publish-web-tooling ../web-tooling-report.json` publishes the compressed report plus its UTC daily pointer to the separate `web-tooling/` Vercel Blob namespace when `BLOB_READ_WRITE_TOKEN` is configured.
