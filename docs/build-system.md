...
**`jetstream`** (needs build, ubuntu-latest x64 only) — Downloads the same production Goccia binary, installs QuickJS and Node Current, sparse-checks out the exact upstream files named by `perf/jetstream/manifest.json` at its pinned JetStream 3.0 commit, and runs the six-workload shell-portable report with five process repetitions per engine. The job always uploads a structurally valid `jetstream-report`, including first-class workload failures, then fails if the report is incomplete. On main it publishes the compressed report plus UTC daily pointer under the independent `jetstream/` Blob namespace. The website combines retained JetStream and AWFY history at `/performance`.
...
**`jetstream-report`** (needs build) — Runs the frozen six-workload JetStream report under the PR Goccia bytecode loader, QuickJS, and Node Current, preserving the pinned upstream sources and five process samples per engine. It uploads the normalized report even when a workload has a first-class failure. The downstream `jetstream-comment` job posts or updates a `JetStream 3 Performance Barometer` comment with separate QuickJS and Node.js reference ratios.
