...
Strings up to 13 bytes are stored inline as `svkString` using `TSouffleInlineString = string[13]`. This avoids heap allocation for the vast majority of strings (property names, short literals, single characters). Strings longer than 13 bytes are stored as `svkReference` pointing to a `TSouffleHeapString` heap object. Note that FPC's `string[13]` is a byte-counted ShortString, so UTF-8 multi-byte sequences (e.g., emoji, CJK characters) may exceed the inline limit even if they appear short in terms of visible characters.
