...
**2026-04-20** · `bytecode` · [#356](https://github.com/frostney/GocciaScript/pull/356) — Call stack depth limit with heap-resident trampoline. Added a configurable call depth limit (`--stack-size=N`, default 3 500) that throws `RangeError` when exceeded. The bytecode VM dispatches bytecode-to-bytecode calls iteratively via an explicit frame stack (`FFrameStack`) on the heap rather than recursing on the Pascal call stack. This is a different approach from V8/JSC, which check the native stack pointer against a guard address — their limit is the OS thread stack size, ours is heap memory. The tradeoff: per-call overhead is higher (~25x vs JIT engines) because each frame transition goes through the switch dispatcher, but call depth is unbounded by OS stack limits. The trampoline is compatible with a future baseline JIT (the JIT would compile per-frame code while frame transitions stay iterative). The interpreter mode retains Pascal recursion with the depth check as a guard. [bytecode-vm.md](bytecode-vm.md). [embedding.md § Call Stack Depth Limit](embedding.md#call-stack-depth-limit).
