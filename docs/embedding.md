...
- On successful execution, the engine's pending `fetch()` requests complete before `Execute` returns; if execution throws, they are detached and their late completions are discarded. Requests started by another engine on the same thread are left alone. The microtask queue is still only used for Promise reactions, not for network I/O.
