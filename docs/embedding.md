...
The engine initializes a mark-and-sweep garbage collector (`TGenericGarbageCollector`) automatically. In most embedding scenarios, no manual GC interaction is needed. The GC collects unreachable values during execution.
...
```pascal
TGenericGarbageCollector.Instance.AddTempRoot(MyValue);
try
  Engine.Execute; // MyValue won't be collected during execution
finally
  TGenericGarbageCollector.Instance.RemoveTempRoot(MyValue);
end;
```
