/*---
description: Class call forms work when property-name constants exceed one byte
features: [class-inheritance, private-methods, bytecode]
---*/

test("super constructor calls support large constant pools", () => {
  class Base {
    constructor() {
      this.base = true;
    }
  ...
  expect(instance.count).toBe(260);
});

test("super method calls support large constant pools", () => {
  class Base {
    overflowMethod() {
      return 7;
    }
  ...
  class Derived extends Base {
    read() {
      ...
      return super.overflowMethod() + count;
    }
  }

  expect(new Derived().read()).toBe(267);
});

test("private method calls support large constant pools", () => {
  class Counter {
    #overflowPrivate() {
      return 3;
    }
    ...
    read() {
      ...
      return this.#overflowPrivate() + count;
    }
  }

  expect(new Counter().read()).toBe(263);
});

test("private field reads and writes support large constant pools", () => {
  class Counter {
    #value = 1;
    ...
    update() {
      ...
      this.#value = previous + count;
      this.#value += 1;
      return this.#value;
    }
  }

  expect(new Counter().update()).toBe(262);
});
