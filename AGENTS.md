...
| GocciaScript Backend | `Goccia.Engine.Backend.pas` | `TGocciaSouffleBackend` — bridges GocciaScript engine to Souffle VM; initializes the Souffle GC singleton (with automatic collection disabled), collects on destruction, shuts down in `finalization` |
