...
| Souffle GC | *(removed — unified into `TGenericGarbageCollector`)* | Souffle heap objects are registered directly with the unified GC via `TGenericGarbageCollector.Instance.AllocateObject` |
...
- **Mark-and-sweep** for garbage collection (`TGenericGarbageCollector` base with unified singleton, `TGocciaGarbageCollector` for scope management; one GC for both interpreter and VM)
