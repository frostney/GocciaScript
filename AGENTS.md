...
- **Parser file inputs must preserve UTF-8 bytes on Windows** — when a parser consumes file-backed text (TOML/JSON/YAML/import maps/globals/source files), do not route UTF-8 through `string(UTF8String(...))` or bare `TStringList.LoadFromFile` and assume Windows will do the right thing. Use the shared UTF-8 file-reading helper so Unicode keys/values survive on both `i386-win32` and `x86_64-win64`, and add one regression that exercises a real file on disk instead of only in-memory strings.
