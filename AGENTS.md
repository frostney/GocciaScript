...
| GocciaScript Runtime | `Goccia.Runtime.Operations.pas` | `TGocciaRuntimeOperations` — GocciaScript semantics for Souffle VM, `ExtendedOperation` handler |

**Souffle VM known limitations:** The bytecode backend passes 100% of the test suite (3,347 tests). All language features work through a bridge layer that delegates to the GocciaScript evaluator for module imports, async/await, iteration, and built-in method calls. Complex classes and classes extending built-in constructors (`Array`, `Map`, `Set`, `Promise`, `Object`) are deferred to the interpreter via `FPendingClasses` / `GOCCIA_EXT_EVAL_CLASS`. `.sbc` binary format uses native endianness (not yet portable). ABC-encoded instructions limit constant pool references to 255 per prototype. See [docs/souffle-vm.md § Known Limitations](docs/souffle-vm.md#known-limitations) for the full list.
