...
| Garbage Collector | `GarbageCollector.Generic.pas` | `TGarbageCollector` — singleton mark-and-sweep GC with generation-counter mark tracking (`Initialize`, `Instance`, `Shutdown`); manages all objects (values, scopes, heap objects) in one pool; O(1) `UnregisterObject`; methods: `RegisterObject`, `UnregisterObject`, `PinObject`, `UnpinObject`, `AddTempRoot`, `RemoveTempRoot`, `IsTempRoot`, `AddRootObject`, `RemoveRootObject`, `PushActiveRoot`, `PopActiveRoot`, `AddExternalRootMarker`, `RemoveExternalRootMarker`, `AllocateObject`, `Collect`, `CollectIfNeeded` |
