{
  TOrderedStringMap<TValue> - Insertion-order-preserving string-keyed map.

  Standalone implementation inheriting from TBaseMap<string, TValue>.
  Hash and equality are static inline — no virtual dispatch on the
  hot lookup path. This restores the original OrderedMap performance
  profile where DJB2 hashing was inlined into FindBucket.
...
{ Hash — static inline: DJB2 over string characters }
...
{ Probe — direct string equality, no method call }
