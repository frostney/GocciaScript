...
suite("Set operations", () => {
  bench("add 50 elements", () => {
    const s = new Set();
    Array.from({ length: 50 }, (_, i) => i).forEach((i) => s.add(i));
  });
  ...
  bench("delete elements", () => {
    const s = new Set(Array.from({ length: 20 }, (_, i) => i));
    s.delete(0);
    s.delete(5);
    s.delete(10);
    s.delete(15);
...
suite("Map operations", () => {
  bench("set 50 entries", () => {
    const m = new Map();
    Array.from({ length: 50 }, (_, i) => i).forEach((i) => m.set("key" + i, i));
  });

  bench("get lookup (50 entries)", () => {
    const entries = Array.from({ length: 50 }, (_, i) => ["key" + i, i]);
  ...
  bench("keys/values/entries", () => {
    const entries = Array.from({ length: 30 }, (_, i) => ["key" + i, i]);
    const m = new Map(entries);
    const keys = [...m.keys()];
    const vals = [...m.values()];
