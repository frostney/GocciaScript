...
// Maintainer-only adapter: normal compliance runs execute the committed
// generated JavaScript suite and do not require Node.js.
...
if (process.argv.length !== 5) {
  console.error(
    "Usage: regenerate-json5-tests.js <json5-repo-dir> <revision> <output>",
  );
  process.exit(2);
}
...
const checkoutRevision = execFileSync(
  "git",
  ["-C", suiteDir, "rev-parse", "HEAD"],
  { encoding: "utf8" },
).trim();

if (checkoutRevision !== revision) {
  throw new Error(
    `Suite revision mismatch: expected ${revision}, got ${checkoutRevision}`,
  );
}
...
const serializedCases = JSON.stringify(cases)
  .replaceAll("\u2028", "\\u2028")
  .replaceAll("\u2029", "\\u2029");
const generatedSuite = `// Generated by scripts/regenerate-json5-tests.js.
// Upstream json5/json5 revision: ${revision}
...
fs.writeFileSync(outputPath, generatedSuite, "utf8");
