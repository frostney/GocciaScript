#!/usr/bin/env bun
/**
 * yaml-test-bump-pin.ts
 *
 * Update the yaml-test-suite SHA pin in `scripts/run_yaml_test_suite.py`.
 * Invoked by the weekly `.github/workflows/yaml-test-bump.yml` cron job.
 *
 * Usage:
 *   bun scripts/yaml-test-bump-pin.ts <new-sha>
 */

import { readFileSync, writeFileSync } from "fs";

const SHA_RE = /^[0-9a-f]{40}$/;
const TARGET = "scripts/run_yaml_test_suite.py";

const SUITE_SHA_RE = /(SUITE_SHA\s*=\s*")([0-9a-f]{40})(")/g;

function main(argv: string[]): number {
  if (argv.length !== 1 || !SHA_RE.test(argv[0])) {
    console.error("Usage: bun scripts/yaml-test-bump-pin.ts <40-hex-sha>");
    return 2;
  }
  const newSha = argv[0];
