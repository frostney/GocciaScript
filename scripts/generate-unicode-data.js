...
// ${RESOURCE_NAME} container ships as a Pascal typed constant instead
// (a linear-memory data segment). Byte-identical to the RCDATA
// payload of ${resourceReference}.
{$I ${unitName}.Blob.inc}
{$ENDIF}
...
// The same resource container bytes as a Pascal typed-constant include
// (the lakon/WASM embedding path; see generatePascalUnit's LAKON block).
function generatePascalBlobInclude(resource) {
  const lines = [];
  lines.push("// Generated by scripts/generate-unicode-data.js (--pascal-blob)");
  lines.push("// The GOCCIA_UCD RCDATA payload as a Pascal typed constant for");
  lines.push("// compilers without Windows-resource embedding (the lakon WASM");
  lines.push("// lane). Byte-identical to the blob inside Generated.UnicodeData.res.");
  lines.push("const");
  lines.push(
    `  GeneratedUnicodeDataResourceBlob: array[0..${resource.length - 1}] of Byte = (`,
  );
  for (let start = 0; start < resource.length; start += 16) {
    const row = [];
    for (let i = start; i < Math.min(start + 16, resource.length); i += 1) {
      row.push(String(resource[i]));
    }
    const trailing = start + 16 < resource.length ? "," : "";
    lines.push(`    ${row.join(", ")}${trailing}`);
  }
  lines.push("  );");
  return `${lines.join("\n")}\n`;
}
...
async function main() {
  ...
  const blobIncludeFile = outputFile.replace(/\.pas$/, ".Blob.inc");
  console.log(`Writing ${path.basename(blobIncludeFile)}...`);
  fs.writeFileSync(blobIncludeFile, generatePascalBlobInclude(resource), "utf8");
